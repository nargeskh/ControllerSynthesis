...
public class MainCS {
	 ...
	 */
	private static  ArrayList<LTS> buildComponentModel() throws IOException, NoStateExistException {
		...
		out.flush();
		putIntoFile(logger.draw(), "logger");
		putIntoFile(receiver.draw(),"receiver");
		putIntoFile(logger.draw(), "logger");
		putIntoFile(cache.draw(), "cache");
		putIntoFile(dispatcher.draw(), "dispatcher");
		putIntoFile(server1.draw(), "server1");
		putIntoFile(server2.draw(), "server2");
		putIntoFile(server3.draw(), "server3");
		putIntoFile(DBA.draw(), "DBA");
		putIntoFile(agg.draw(), "agg");
	...
	private static Graph getHeavyLoadStructure()
	{
	...
	private static Graph getLightLoadStructure()
	{
	...
	private static void test_synthesise_phase2() throws IOException, NoStateExistException {
		...
		ArrayList<LTS>  ltslist = buildComponentModel();
		Graph model = getHeavyLoadStructure();
	...
	private static void printEachConfigAdaptor(gLTS adaptor, gLTS plan) {
		for(gState s:plan.states.values())
		{
			...
			putIntoFile( configAdaptor.draw(), "adaptor"+s.s.ID);
		}
